...
module.exports = function (HttpSerializer) {
  ...
  function (contextResponse, request, response) {
    ...
    if (payload != null)
      contextResponse.payload = JSON.stringify(payload.records,
        function (key, value) {
    ...
    for (i = 0, j = records.length; i < j; i++) {
      ...
      for (field in record) {
        ...
        record[field] = Array.isArray(value) ?
          value.map(curryCast(castValue, fieldType, Object.assign(opts, options))) :
          castValue(value, fieldType, Object.assign(opts, options))
      }
    }
